...
int ava_init(ava* p); // Прототип функции инициализации структуры ava
int ava_close(ava* p); // Прототип функции закрытия avaxxxx.dat
int findChNumb(rgset* pr, ava* ap, int i); // Функция определения номера канала в rgset.dat по номеру канала в avaxxxx.dat
int CpxToGrp(CpxGrp complrx); // Функция перевода номера группы из комплексного в машинный формат
CpxGrp GrpToCpx(int MashGrp); // Функция перевода номера группы из машинного формата в комплексный
int ava_bin_init(ava* p);  // Инициализация (разворачивание двоичных сработок из ava_bin в ava_bin_i)
void ava_bin_close(ava* p); // Закрытие двоичных сработок, очистка памяти (удаление объектов ava_bin_i)
#endif
